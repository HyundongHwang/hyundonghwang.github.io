---
layout: post
title: 260612 Firestore 문서처리 DB를 Aurora PostgreSQL S3로 전환하기
comments: true
tags:
- aws
- postgresql
...
# 260612 Firestore 문서처리 DB를 Aurora PostgreSQL + S3로 전환하기

샘플 문서처리 프로젝트는 뉴스, 리포트, PDF, HTML 같은 문서 원문을 수집하고 요약, 감정분석, 통계, 검색으로 확장하려는 구조다. 초기에는 Firestore가 빠르고 편하지만, 문서량이 커지고 분석 작업이 늘어나면 `Aurora PostgreSQL + S3 + SQLModel` 조합이 더 적합해진다.
...
샘플 프로젝트의 `MyDoc` 성격 데이터는 다음과 같다.

| 데이터 | 특징 | Firestore 단독 사용 시 이슈 |
|---|---|---|
| 문서/리포트/PDF 본문 | 길어질 수 있음 | 문서 1MiB 제한 |
| 감정분석 결과 | 배열/객체 구조 | 인덱스 fanout 가능 |
| 상태값 | `NONE`, `ANALYZING`, `ANALYZED` | 대량 상태 조회/갱신 비용 |
| 통계 | category/type/state count | 반복 read 비용 |
...
select *
from my_doc
where meta->>'provider' = 'SAMPLE';
```

```sql
create index idx_my_doc_meta_gin
on my_doc using gin (meta);
```

`MyDoc` 데이터의 `meta`, `sentiment_list`는 PostgreSQL에서 `jsonb`로 옮기기 좋다. 다만 자주 조인하거나 집계해야 하는 분석 결과는 별도 테이블로 정규화하는 편이 낫다.
...
```text
my-doc/raw/{category}/{type}/{provider}/{yyyy}/{mm}/{doc_id}.md
my-doc/raw/GENERAL/REPORT/SAMPLE/2026/06/GENERAL_REPORT_SAMPLE_001.md
my-doc/raw/GENERAL/PDF/SAMPLE/2026/06/GENERAL_PDF_SAMPLE_002.md
```
...
create index idx_my_doc_category_type_publish
on my_doc (category, type, dt_publish desc);

create index idx_my_doc_sentiment_state
on my_doc (sentiment_state);
...
```sql
create table my_doc_chunk (
    id bigserial primary key,
    my_doc_id text not null references my_doc(id) on delete cascade,
    chunk_index integer not null,
    content text not null,
    token_count integer not null default 0,
    meta jsonb not null default '{}',
    dt_create timestamptz not null default now(),
    unique (my_doc_id, chunk_index)
);
```

## Python ORM은 SQLModel 추천

Pydantic 기반 DTO를 사용하는 Python 프로젝트라면 `SQLModel`이 잘 맞는다.
...
class MyDoc(SQLModel, table=True):
    __tablename__ = "my_doc"
...
DATABASE_URL = os.environ["MY_DB_URL"]
...
S3_BUCKET = os.environ["MY_S3_BUCKET"]
...
def get_my_doc(session: Session, doc_id: str) -> MyDoc | None:
    return session.get(MyDoc, doc_id)


def list_pending_docs(session: Session, limit: int = 100) -> list[MyDoc]:
    stmt = (
        select(MyDoc)
        .where(MyDoc.sentiment_state == "NONE")
        .order_by(MyDoc.dt_publish.desc())
        .limit(limit)
    )
...
def upsert_my_doc(session: Session, data: dict) -> MyDoc:
    now = datetime.utcnow()
    data["dt_update"] = now

    stmt = insert(MyDoc).values(**data)
    update_data = {key: value for key, value in data.items() if key not in {"id", "dt_create"}}

    stmt = stmt.on_conflict_do_update(
        index_elements=[MyDoc.id],
        set_=update_data,
    )
    ...
    doc = session.get(MyDoc, data["id"])
    if doc is None:
        raise RuntimeError(f"MyDoc upsert failed: {data['id']}")
...
Firestore는 초기 개발 속도와 운영 단순성 면에서 강하다. 하지만 문서량이 많고, 원문 저장, 감정분석, 요약, 통계, 검색이 중요한 프로젝트는 PostgreSQL 쪽 장점이 빠르게 커진다. 현실적인 방향은 Firestore를 보조 저장소로 남기고, 문서처리 핵심 파이프라인은 `Aurora PostgreSQL + S3 + SQLModel`로 분리하는 것이다.
