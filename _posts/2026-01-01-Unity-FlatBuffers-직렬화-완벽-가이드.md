...
namespace [MyGame.Data](http://MyGame.Data);
...
```
namespace [MyGame.Network](http://MyGame.Network);
...
```
using FlatBuffers;
using [MyGame.Data](http://MyGame.Data);
...
public class CharacterSerializer : MonoBehaviour
{
    public byte[] SerializeCharacter(PlayerCharacter player)
    {
        ...
        // 1. 문자열 생성
        var nameOffset = builder.CreateString([player.name](http://player.name));
        ...
        // 4. 인벤토리 벡터 생성
        var inventoryIds = [player.inventory.Select](http://player.inventory.Select)(item => item.itemId).ToArray();
        var inventoryCounts = [player.inventory.Select](http://player.inventory.Select)(item => item.count).ToArray();
        ...
        Character.AddGold(builder, [player.gold](http://player.gold));
...
public Monster DeserializeMonster(byte[] data)
{
    ...
    // 3. 데이터 사용 (즉시 접근 가능)
    Debug.Log($"Name: {[monster.Name](http://monster.Name)}");
...
⏺ // Version 1
  ...
  주요 변경사항:
  - 모든 들여쓰기를 space 4로 통일
  - 버전 주석과 테이블 정의 사이 빈줄 제거
  - 두 테이블 정의와 설명 주석 사이에 빈줄 1개씩만 유지
  - 필드 정의 정렬 통일
  - 인라인 주석 위치 조정
  - 여러 줄 주석을 연속된 단일 라인 주석으로 정리
```
...
<linker>
    ...
    <namespace fullname="[MyGame.Data](http://MyGame.Data)" preserve="all"/>
    <namespace fullname="[MyGame.Network](http://MyGame.Network)" preserve="all"/>
    </assembly>
</linker>
```
...
⏺ using UnityEngine;
  ...
  public class PlatformTest : MonoBehaviour
  {
      ...
      private void TestFlatBuffers()
      {
          ...
          {
              Debug.LogError($"❌ FlatBuffers failed: {ex.Message}\n{ex.StackTrace}");
          }
      }
  }
```
...
- [FlatBuffers in .NET - [GameDev.net](http://GameDev.net)](https://gamedev.net/blogs/entry/2259791-flatbuffers-in-net/)
